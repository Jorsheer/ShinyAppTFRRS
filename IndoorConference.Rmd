...
inputPanel(
  ...
  sliderInput("athletes1", label = "Number of Athletes",
      min = 1, max = 50, value = 25, step = 1)
)
...
inputPanel(
  selectInput("year3", "Select Year:", choices = sort(unique(final_miac_indoor$Year)), selected = max(final_miac_indoor$Year)),
  selectInput("event3", "Select Event:", choices = sort(unique(final_miac_indoor$Event)), selected = "60 Meters (Men)"),
  sliderInput("athletes3", label = "Number of Athletes",
      min = 1, max = 50, value = 25, step = 1),
  selectInput("team3", "Select Team:", choices = sort(unique(final_miac_indoor$Team)), selected = "St. Olaf")
)

renderPlot({
  
  filtered_data <- final_miac_indoor |>
    filter(Year == input$year3, Event == input$event3)
  
  filtered_data <- filtered_data |>
    mutate(point_color = ifelse(Team == input$team3, "gold", "gray"))
    
  top_n <- input$athletes3
  if (any(!is.na(filtered_data$Time))) {
    filtered_data <- filtered_data |>
      arrange(Time) |>
      head(top_n)
...
inputPanel(
  selectInput("event4", "Select Event:", choices = sort(unique(final_miac_indoor$Event)), selected = "60 Meters (Men)"),
  sliderInput("athletes4", label = "Number of Athletes",
      min = 1, max = 50, value = 25, step = 1),
  checkboxGroupInput("selected_years4", "Select Years:",
...
renderPlot({
  ...
  event_data <- final_miac_indoor |>
    filter(Event == input$event4, Year %in% input$selected_years4)

  top_n <- input$athletes4
