...
inputPanel(
  selectInput("year1", "Select Year 1:", choices = sort(unique(final_miac$Year)), selected = max(final_miac$Year)),
  selectInput("year2", "Select Year 2:", choices = sort(unique(final_miac$Year)), selected = max(final_miac$Year)),
  selectInput("event", "Select Event:", choices = sort(unique(final_miac$Event)), selected = "60 Meters (Men)"),
  sliderInput("athletes", label = "Number of Athletes",
      min = 1, max = 50, value = 25, step = 1)
)
...
inputPanel(
  ...
  sliderInput("athletes", label = "Number of Athletes",
      min = 1, max = 50, value = 25, step = 1),
  selectInput("team", "Select Team:", choices = sort(unique(final_miac$Team)), selected = "St. Olaf")
)
...
inputPanel(
  ...
  checkboxGroupInput("selected_years", "Select Years:",
                     choices = sort(unique(final_miac$Year)),
                     selected = sort(unique(final_miac$Year)))
)

renderPlot({
  
  event_data <- final_miac |>
